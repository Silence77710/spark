 ...
 **自动记录规则**：
 ...
 - **卡片内容**：标题、内容预览（截取前 100 字）、状态标签、重要程度标签（等级 > 0 时）、创建时间（相对时间如"3分钟前"）
 ...
 **想法墓志铭**：详情页底部展示已有墓志铭（如有，斜体），详见 3.9
 ...
 - **捕获时**：默认为 0（未评级），不强制选择
 - **列表页**：
   - 卡片右上角显示重要程度标签（0 不显示，避免噪音）
   - 支持按等级多选筛选
   - 透镜预设：`重点`（3-4）、`未整理`（0）；手动多选后透镜自动取消高亮
...
*文档版本：v1.3 · 2026-08-18（新增 P1 功能规格：AI 连接器、想法杂交台、AI 回望对话、AI 隐私控制完整版）*
