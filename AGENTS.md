...
- **本文件 (`AGENTS.md`) 是 Codex（CLI / IDE / Desktop）与 Cursor 0.45+ 自动加载的项目级规则入口**。
- AI 工具读到本文件后，必须按下方「文档同步（硬规则）」段（第 17–26 行）的 8 条触发条件执行：触发条件命中时**强制**在同一次变更里同步更新对应文档。
- 详细执行细则（提交前自检清单、协作流程、违反后果）见 [`docs/规范.md`](./docs/规范.md)；AI 工具应在涉及文档同步、提交前自检、违反后果判定时主动加载该文档。
- Cursor 0.45+ 用户可额外加载 `.cursor/rules/documentation-sync.mdc` 以获得更精细的规则触发。
